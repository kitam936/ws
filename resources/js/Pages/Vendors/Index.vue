<script setup>
    ...
    const searchVendors = () => {
        router.get(route('vendors.index', { search: search.value }))
    }

    const resetFilters = () => {
        search.value = '';

        router.get(route('vendors.index'), { preserveState: true, preserveScroll: true });
    }
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            ...
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">仕入先一覧</h2>
            ...
            <div class="flex">
                ...
                <div  v-if="login_user.role_id <= 2" class="ml-2 md:ml-24 mt-4">
                    <Link as="button" :href="route('vendors.create')" class="w-40 h-10 bg-green-500 text-sm text-white ml-0 hover:bg-green-600 rounded">仕入先登録</Link>
                </div>
            </div>
        </template>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    ...
                    <div class="p-2 text-gray-900">
                        ...
                        <Pagination :links="vendors.links" class="mt-4" ></Pagination>
                        </div>
                    </div>
                </div>
            </div>
